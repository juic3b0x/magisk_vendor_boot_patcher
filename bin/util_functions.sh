...
install_magisk() {
  ...
  flash_image new-vendor_boot.img "$BOOTIMAGE"
  case $? in
    1)
...
sign_chromeos() {
  ...
  echo > empty
  ./chromeos/futility vbutil_kernel --pack new-vendor_boot.img.signed \
  --keyblock ./chromeos/kernel.keyblock --signprivate ./chromeos/kernel_data_key.vbprivk \
  --version 1 --vmlinuz new-vendor_boot.img --config empty --arch arm --bootloader empty --flags 0x1

  rm -f empty new-vendor_boot.img
  mv new-vendor_boot.img.signed new-vendor_boot.img
}
...
run_migrations() {
  ...
  for gz in /data/stock_boot*.gz; do
    ...
    mv $gz /data/magisk_backup_${LOCSHA1}/vendor_boot.img.gz
  done
