...
if [ -c "$BOOTIMAGE" ]; then
  nanddump -f vendor_boot.img "$BOOTIMAGE"
  BOOTNAND="$BOOTIMAGE"
  BOOTIMAGE=vendor_boot.img
fi
...
./magiskboot unpack --vendor "$BOOTIMAGE"
...
if [ -e vendor_ramdisk_.cpio ]; then
  ./magiskboot cpio vendor_ramdisk_.cpio test
  STATUS=$?
  SKIP_BACKUP=""
else
...
case $((STATUS & 3)) in
  0 )  # Stock boot
    ...
    cat $BOOTIMAGE > stock_vendor_boot.img
    cp -af vendor_ramdisk_.cpio vendor_ramdisk_.cpio.orig 2>/dev/null
    ;;
  1 )  # Magisk patched
    ui_print "- Magisk patched boot image detected"
    ./magiskboot cpio vendor_ramdisk_.cpio \
    "extract .backup/.magisk config.orig" \
    "restore"
    cp -af vendor_ramdisk_.cpio vendor_ramdisk_.cpio.orig
    rm -f stock_vendor_boot.img
    ;;
  2 )  # Unsupported
...
"patch" \
"$SKIP_BACKUP backup vendor_ramdisk_.cpio.orig" \
"mkdir 000 .backup" \
"add 000 .backup/.magisk config" \
|| abort "! Unable to patch ramdisk"

rm -f vendor_ramdisk_.cpio.orig config magisk*.xz stub.xz
